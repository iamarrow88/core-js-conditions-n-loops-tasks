...
function shuffleChar(str, iterations) {
  function shuffle(strInner) {
    let odds = '';
    let evens = '';

    for (let i = 0; i < strInner.length; i += 1) {
      if (i % 2 === 0) {
        evens += strInner[i];
      } else {
        odds += strInner[i];
...
function getNearestBigger(number) {
  const basicNumberToCompare = number
    .toFixed(0)
    .split('')
    .sort((a, b) => a - b)
    .join('');
  ...
  let secondNumberToCompare = number.toFixed(0);
  do {
    secondNumber += 1;
    secondNumberToCompare = secondNumber
      .toFixed(0)
      .split('')
      .sort((a, b) => a - b)
      .join('');
  } while (basicNumberToCompare !== secondNumberToCompare);
  return +secondNumber;
}
